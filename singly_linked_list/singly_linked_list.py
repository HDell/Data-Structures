...
""" My Failed Attempt at Implementing a LinkedList from Scratch 
...
class LinkedList:
    def __init__(self, head=Node, tail=Node): #This may have been the point of failure
        self.head = head
        self.tail = tail
    ...
    def get_max(self):
        curr_max = None
        if self.head is not None: #if LL isn't empty
            current_node = self.head
            curr_max = current_node.value
            while current_node is not self.tail:
                if current_node.next_node.value > curr_max:
                    curr_max = current_node.next_node.value
                    current_node = current_node.next_node
                else:
                    current_node = current_node.next_node
            return curr_max
